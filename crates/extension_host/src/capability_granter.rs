...
impl CapabilityGranter {
    ...
    pub fn grant_ui_redwood(&self) -> Result<()> {
        let is_allowed = self
            .granted_capabilities
            .iter()
            .any(|capability| matches!(capability, ExtensionCapability::RedwoodUi));

        if !is_allowed {
            bail!("capability for ui:redwood is not granted by the extension host");
        }
        Ok(())
    }
}
