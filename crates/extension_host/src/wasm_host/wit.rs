...
impl Extension {
    ...
    pub async fn call_init_extension(&self, store: &mut Store<WasmState>) -> Result<()> {
        match self {
            Extension::V1_0_0(ext) => ext.call_init_extension(store).await,
    ...
    pub async fn call_suggest_docs_packages(
        ...
        match self {
            Extension::V1_0_0(ext) => ext.call_suggest_docs_packages(store, provider).await,
