...
pub const MIN_VERSION: SemanticVersion = SemanticVersion::new(1, 0, 0);
...
// UI host functions: minimal bridge to GPUI preview renderer.
impl ui::Host for WasmState {
    async fn create_panel(&mut self, panel_id: u64, title: String) -> wasmtime::Result<()> {
        log::info!(target: "zed_ext_ui", "create-panel id={} title={}", panel_id, title);
        Ok(())
    }

    async fn close_panel(&mut self, panel_id: u64) -> wasmtime::Result<()> {
        log::info!(target: "zed_ext_ui", "close-panel id={}", panel_id);
        Ok(())
    }
...
// Key/value store, project, and worktree resources.
